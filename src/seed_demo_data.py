...
HOTSPOTS = [
    ...
    # -- OpenSky-flagged aviation disruption (grounded/reduced national-carrier
    #    fleets during conflict), matched the same as any other Phase 1 signal --
    ("AF", "conflict", "military_conflict", "OpenSky Network", (6, 120), (0.4, 0.8), 3),
]
...
UNEXPLAINED_ONLY = [
    ...
    ("AR", "OpenSky Network", (1, 6), (0.1, 0.3), 2),
]

# OpenSky-detected aviation disruption paired with FAA NOTAM restriction
# records — US-scoped (FAA is US-only, see src/ingestion/faa_notam.py), so
# this stands apart from the HOTSPOTS loop above, which only ever emits
# ACLED/GDACS/#KeepItOn-sourced cause_events. Wildfire/hurricane TFRs
# (Temporary Flight Restrictions) are routine, non-sensitive, and don't
# assert anything about a specific real event -- dates are randomized same
# as everywhere else in this file.
US_AVIATION_SCENARIOS = [
    ("disaster", "wildfire_tfr", (2, 48), (0.2, 0.5), 2),
    ("disaster", "hurricane_tfr", (4, 72), (0.3, 0.6), 2),
]
...
def generate(lookback_days: int = 90) -> tuple[list[dict], list[dict]]:
    ...
    us_geo_hit = geo.lookup("US")
    if us_geo_hit:
        us_name, us_lat, us_lon = us_geo_hit
        for i, (cause, subtype, dur_range, sev_range) in enumerate(
            [(c, s, d, sv) for c, s, d, sv, n in US_AVIATION_SCENARIOS for _ in range(n)]
        ):
            start = now - timedelta(days=random.uniform(0, lookback_days))
            duration_hours = round(random.uniform(*dur_range), 1)
            end = start + timedelta(hours=duration_hours)
            outages.append(
                {
                    "event_id": f"seed:outage:US:{subtype}:{i}:{int(start.timestamp())}",
                    "lat": round(_jitter(us_lat, 3.0), 3),
                    "lon": round(_jitter(us_lon, 3.0), 3),
                    "region_name": us_name,
                    "country": us_name,
                    "asn": None,
                    "timestamp_start": start.isoformat(),
                    ...
                    "cause_subtype": None,
                    "source_type": "structured",
                    "source_name": "OpenSky Network (seed)",
                    "confidence": "low",
                    "severity_score": round(random.uniform(*sev_range), 2),
                }
            )
            if random.random() < CAUSE_EVENT_MATCH_PROBABILITY:
                ...
                cause_events.append(
                    {
                        "cause_event_id": f"seed:notam:US:{subtype}:{i}",
                        "source": "notam",
                        "cause": cause,
                        "cause_subtype": subtype,
                        "country": us_name,
                        "lat": round(_jitter(us_lat, 3.0), 3),
                        "lon": round(_jitter(us_lon, 3.0), 3),
                        "event_date": (start + timedelta(hours=offset_hours)).isoformat(),
                        "title": f"Seed FAA NOTAM record ({subtype.replace('_', ' ')}) — {us_name}",
                        "raw": {"seed": True},
                    }
                )
